...
class Email {
  ...
  static sendRecover(user) {
    return new Promise((resolve, reject) => {
      ...
      //guarda en redis
      let link = `http://localhost:3000/api/usuarios/recuperarContraseña?token=${token}`
      let mailOptions = {
        from: config.email.from,
        to: user.email,
        subject: "Servicio de recuperación de contraseña ",
        html: "Hola,<br> Please Click on the link to verify your email.<br><a href=" + link + ">Click here to verify</a>"
      }
      smtpTransport.sendMail(mailOptions, function (error, response) {
        if (error) {
          console.log(error);
